...
class Swarm:
    ...
    def __init__(self, height, width, number, clock_speed,
        ...
        self.leds_X_positions = np.random.randint(low=0, high=width, size=number)
        self.leds_Y_positions = np.random.randint(low=0, high=height, size=number)
        self.leds_clocks = np.random.rand(number)
    ...
    def update_clocks(self):
        if self.nudge_on :
            ...
            is_shiny_leds = (self.leds_clocks > - self.leds_clock_speed) & (self.leds_clocks < self.leds_clock_speed) & (self.leds_on)
    ...
    def update_leds(self):
        self.leds_clocks = self.leds_clocks + self.leds_on * self.leds_clock_speed